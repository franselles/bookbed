...
const { zeroPad, zeroUnpad } = require('./zero-pad');
...
class Redsys {
  ...
  createMerchantSignatureNotif(key, data) {
    ...
    const orderId = _data.Ds_Order || _data.DS_ORDER || _data.DS_MERCHANT_ORDER;
    const orderKey = this.encrypt3DES(orderId, key);
  ...
  /**
   * Check if signatures before and after redirection are the same
   * @param {*} signA 
   * @param {*} signB 
   */
